...
# File: task2.py
# 
# Description: Main driver file for Task 2 of the assignment.
##################################################################################
...
def main(*args):
    """
    Main function for Task 2 of Phase 3 of the CSE 515 Fall 2021 project.
    """
    ...
    labels=[int(each.split("-")[2]) for each in image_dict.keys()]
    ...
    expected_labels = [int(each.split("-")[2]) for each in image_dict_unlabeled.keys()]
    ...
    if classifier == 'svm':
        data_x_copy, _, data_y_copy, _ = train_test_split(data_x, data_y, test_size=0.5, random_state=42)
        m=len(data_x_copy)
        c=len(np.unique(data_y_copy))
        

        X_train, _, y_train, _ = train_test_split(data_x, data_y, test_size=0.01, random_state=42)
        W=svmMultiClass.fit(X_train,pd.DataFrame(y_train))
        y_predicted,_ = svmMultiClass.predict(data_x_test,W)
        
        print(y_predicted)
        print(accuracy_score(data_y_test,y_predicted))
        ...
        #print(report)
        file = open(classifier+"_"+"Task2.txt","w")
        ...
        labels_dt=[int(each.split("-")[2]) for each in image_dict.keys()]
        ...
        classified_tuples, potential_labels = ppr.PPR_Classifier(image_dict, comparison_image_dict, 'subject', 0.6, dim_red_technique, top_n=len(image_dict)//40)

    # Compute and print false positive/miss rates
    accuracy_stats, overall_accuracy = stats.compute_statistics(classified_tuples, comparison_image_dict, potential_labels, 'subject')
    # Print/save results and labels to some file
    io.save_classification_results(output_folder, f"2_{classify_folder}_{classifier}.txt", classified_tuples, accuracy_stats, overall_accuracy)
